...
| SIEM      | `/siem/alerts` (GET/POST — sortable/filterable, manual/intel escalation), `/siem/alerts/{id}` (GET/PATCH), `/siem/kpis`, `/siem/correlations`, `/siem/mitre-distribution`, `/siem/rules` (GET/POST), `/siem/rule-schema`, `POST /siem/rules/test` (backtest), `/siem/rules/{id}` (PATCH/DELETE), `/siem/sources` (GET/POST), `/siem/hunts` (GET/POST), `POST /siem/hunts/{id}/run`, `POST /siem/hunt-query` (ad-hoc hunt engine) |
