...
def _seed_iocs(conn, rng, actors):
    ...
    for _ in range(120):
        ...
        conn.execute(
            _INSERT_IOC,
            ioc_row(type=t, value=value,
                    threat_type=rng.choice(["C2", "Phishing", "Malware", "Scanning", "Exfil"]),
                    confidence=rng.randint(40, 99), severity=rng.choice(SEVERITIES),
                    source=rng.choice(["OTX", "abuse.ch", "MISP", "Internal", "VirusTotal"]),
                    actor=rng.choice(actors) if rng.random() > 0.4 else "",
                    first_seen=_ago(rng, 720), last_seen=_ago(rng, 72),
                    tags=rng.sample(["malicious", "active", "confirmed", "high-conf"],
                                    rng.randint(1, 2))),
        )
