...
const LOG_LINES = [
  { text: '192.168.1.45 - - [12/May/2024:14:23:01] "GET /admin/../etc/passwd HTTP/1.1" 404', flag: 'threat' },
  { text: '10.0.0.12 - - [12/May/2024:14:23:02] "POST /api/login HTTP/1.1" 200', flag: null },
  { text: '2024-05-12T14:23:03Z [WARN] sshd: Failed password for root from 45.12.34.67', flag: 'threat' },
...
const DETECTORS = [
  { name: 'Pattern Matching', desc: 'Regex rules for known attack signatures', count: '2 hits', color: 'threat' },
  { name: 'Statistical', desc: 'Z-score deviation from per-source baseline', count: '1 anomaly', color: 'warn' },
  { name: 'ML Model', desc: 'Isolation Forest with rolling features', count: 'Clean', color: 'safe' },
          ...
          >
            <div className="glass border border-white/8 rounded-2xl overflow-hidden">
              <div className="flex items-center gap-2 px-4 py-3 border-b border-white/5 bg-white/2">
                ...
                <span className="ml-auto text-[10px] text-ink-500 font-mono">Apache / 7 lines</span>
              </div>
              <div className="p-4 font-mono text-[10px] space-y-1.5 max-h-[220px] overflow-hidden">
                {LOG_LINES.map((line, i) => (
                  ...
                  >
                    ...
                    {!line.flag && (
                      <CheckCircle className="w-3 h-3 text-ink-600 shrink-0 mt-0.5" strokeWidth={1.5} />
                    )}
                    <span
                      className={
                        line.flag === 'threat'
