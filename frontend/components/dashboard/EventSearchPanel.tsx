    ...
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
      ...
      <div className="flex items-center gap-3 px-5 py-3.5 border-b border-white/5 flex-wrap">
        ...
        <select value={range} onChange={(e) => setRange(e.target.value)}
          className="ml-auto px-2.5 py-1.5 rounded-lg bg-surface-2 border border-white/8 text-[11px] text-ink-200 focus:outline-hidden focus:border-violet/40">
        ...
        <div className="flex items-center gap-2">
          <div className="flex-1 flex items-center gap-2 px-3 py-2 rounded-xl bg-[#080614] border border-white/8 focus-within:border-violet/40 transition-colors">
            ...
            <input value={query} onChange={(e) => setQuery(e.target.value)}
              ...
              className="flex-1 bg-transparent text-[12px] font-mono text-ink-100 focus:outline-hidden placeholder-ink-700" />
          </div>
          ...
          <div className="flex items-center gap-1.5 flex-wrap">
            ...
            {EXAMPLES.map((ex) => (
              <button key={ex} onClick={() => { setQuery(ex); run(ex) }}
                className="text-[10px] font-mono px-2 py-0.5 rounded-sm bg-surface-2 border border-white/8 text-ink-400 hover:text-violet hover:border-violet/30 transition-colors">
                {ex}
              </button>
            ))}
          </div>
        ...
        {res && (res.interpreted.conditions.length > 0 || res.interpreted.freetext.length > 0 || res.interpreted.stats) && (
          <div className="flex items-center gap-1.5 flex-wrap">
            ...
            {res.interpreted.conditions.map((c, i) => (
              <span key={i} className="text-[10px] font-mono px-1.5 py-0.5 rounded-sm bg-violet/10 text-violet border border-violet/20">
                {c.field} {c.op} {c.value}
              </span>
            ))}
            {res.interpreted.freetext.map((f, i) => (
              <span key={`f${i}`} className="text-[10px] font-mono px-1.5 py-0.5 rounded-sm bg-surface-2 text-ink-300 border border-white/10">
                raw~“{f}”
              </span>
            ))}
            {res.interpreted.stats && (
              <span className="text-[10px] font-mono px-1.5 py-0.5 rounded-sm bg-amber/10 text-amber border border-amber/20 flex items-center gap-1">
              ...
              {res.stats.groups.map((g) => (
                <div key={g.value} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 h-4 rounded-sm bg-white/5 overflow-hidden">
                    <div className="h-full rounded-sm bg-violet/50" style={{ width: `${(g.count / maxGroup) * 100}%` }} />
                  </div>
              ...
              {res.results.map((e) => (
                <div key={e.id} className="px-5 py-2.5">
                  <div className="flex items-center gap-2 flex-wrap text-[10px]">
                    ...
                    {e.eventType && <span className="px-1.5 py-0.5 rounded-sm bg-magenta/10 text-magenta font-mono">{e.eventType}</span>}
                    ...
                    {e.mitreTechId && <span className="px-1.5 py-0.5 rounded-sm bg-violet/10 text-violet font-mono">{e.mitreTechId}</span>}
                  </div>
