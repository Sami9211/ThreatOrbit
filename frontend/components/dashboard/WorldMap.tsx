...
/* ───────────────────────────────────────────────────────────────────
   Accurate dot-matrix world map.
   Continents are defined as real [lon, lat] polygons; a land/sea grid is
   sampled by point-in-polygon, and only land cells get a dot. Everything
   (dots, cities, attack arcs) is placed via an equirectangular projection
   so geographic positions are accurate to their true latitude/longitude.
   ─────────────────────────────────────────────────────────────────── */

// viewBox is 360 × 180; x = lon + 180, y = 90 − lat
const W = 360
const H = 180
const project = (lon: number, lat: number): [number, number] => [lon + 180, 90 - lat]

/* Rough but topologically-correct continent outlines ([lon, lat]).
   The dot sampling quantises these, so coarse polygons still read as a
   recognisable world map. */
const LAND: number[][][] = [
  // North America
  ...
  // Asia (Eurasia eastern mass: Middle East → India → SE Asia → Siberia)
...
/* Cities placed by real latitude / longitude */
...
/* Attacks as source → destination city pairs (by real coordinates) */
...
/* quadratic-bezier arc that bows toward the pole between two points */
...
export default function WorldMap() {
  // Sample a 2.5°-longitude × 3°-latitude grid; keep only land cells.
  const dots = useMemo(() => {
  ...
  return (
    <div className="relative w-full h-full bg-[#070512] rounded-xl overflow-hidden border border-white/5">
      ...
      >
        ...
        {/* Land dots */}
        {dots.map((d, i) => (
          <circle key={i} cx={d.x} cy={d.y} r={0.62} fill="#3A2E63" opacity={0.9} />
        ))}

        {/* Attack arcs */}
        {ATTACKS.map((a, i) => {
          const s = cityMap[a.from]
          const t = cityMap[a.to]
          if (!s || !t) return null
      ...
      {/* Legend */}
      <div className="absolute bottom-3 right-3 flex items-center gap-3 text-[10px] text-ink-500">
